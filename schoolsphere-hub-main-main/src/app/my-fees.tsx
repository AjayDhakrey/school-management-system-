      ...
      {effectiveAmount > 0 && (
        <div className="rounded-lg bg-primary/5 border border-primary/20 p-2.5 text-sm">
          ...
          <div className="flex justify-between text-muted-foreground text-xs mt-0.5">
            ...
            <span className={willBeFullyPaid ? "text-success font-medium" : "text-warning font-medium"}>
...
export default function MyFeesPage() {
  ...
  const nextDue = useMemo(
    () => rows.filter((f) => f.status !== "Paid").sort((a, b) => (a.due_date ?? "").localeCompare(b.due_date ?? ""))[0],
    [rows],
  );
  ...
  return (
    <div>
      <PageHeader title="Fees" description="View and pay your school fees." breadcrumb={["Dashboard", "Fees"]} />
      ...
      <div className="mb-4 grid grid-cols-2 gap-3 sm:grid-cols-2 lg:grid-cols-4">
        ...
        <InfoCard label="Next Due Date" value={nextDue?.due_date ?? "—"} icon={Clock3} tone="gold" />
      </div>
      ...
      <div className="panel">
        {isLoading ? (
          ...
          <EmptyState title="No fee records found" description="Your school hasn't added fee records yet." icon={Wallet} />
        ) : (
                    ...
                    <TableRow key={f.id}>
                      ...
                      <TableCell className="text-right">
                        <div className="flex items-center justify-end gap-1.5">
                          {f.status === "Paid" ? (
                            <Button variant="outline" size="sm" onClick={() => setReceipt({ fee: f, receiptNo: f.receipt_no ?? "—" })}>
