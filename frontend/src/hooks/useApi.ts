...
export interface ApiAttendanceRosterStudent {
  ...
  status: string | null;
  remarks: string | null;
  /** When this student's mark was last saved; null while unmarked. */
  saved_at: string | null;
  /** An approved leave covers this date: the status is locked to Leave. */
...
export interface ApiFee {
  ...
  academic_year_id?: string | null;
  class_id?: string | null;
  /** For generated monthly dues, the month covered, e.g. "April 2026" (see lib/fees.ts). */
  description?: string | null;
  total_fee?: number;
...
export interface ApiPayrollItem {
  ...
  fixed_deductions: number;
  net_salary: number;
  status: "DRAFT" | "CALCULATED" | "REVIEWED" | "APPROVED" | "ON_HOLD" | "PAID" | "CANCELLED";
...
export const isUpcomingExam = (exam: Pick<ApiExam, "status">) =>
  exam.status !== "PUBLISHED" && exam.status !== "CANCELLED";
export const isCompletedExam = (exam: Pick<ApiExam, "status">) => exam.status === "PUBLISHED";
...
export interface ApiNotice {
  ...
  date: string | null;
  author: string | null;
  /** Set on a teacher's class notice; null for school-wide notices. */
  class_id: string | null;
  class_name: string | null;
  section: string | null;
  created_by: string | null;
...
/** Who a notice is for: the class of a class notice, otherwise its audience. */
export function noticeAudienceLabel(n: ApiNotice) {
  if (!n.class_id) return n.audience ?? "All";
  // Students and parents can't read class names; they only ever see their own class's notices.
  return n.class_name ? `${n.class_name} ${n.section ?? ""}`.trim() : "Class notice";
}
...
/** A period of one class's timetable, with the name of the teacher taking it. */
export interface ApiClassTimetableSlot {
  id: string;
  day: string;
  period: number;
  subject: string | null;
...
export const useHasPermission = (permission: string, enabled = true) =>
...
export const useClassTimetable = (classId: string | undefined) =>
  useApiQuery<ApiClassTimetableSlot[]>(
    `timetable-class-${classId ?? "none"}`,
...
export const usePayroll = (month?: string, enabled = true) =>
  useApiQuery<ApiPayrollItem[]>(
    `payroll-${month ?? "all"}`,
    `/payroll${month ? `?month=${month}` : ""}`,
    enabled,
  );
