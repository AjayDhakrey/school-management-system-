      ...
      {effectiveAmount > 0 && (
        <div className="rounded-lg bg-primary/5 border border-primary/20 p-2.5 text-sm overflow-hidden">
          ...
          <div className="flex items-center justify-between gap-2 text-muted-foreground text-xs mt-0.5">
            ...
            <span
              className={`shrink-0 ${willBeFullyPaid ? "text-success font-medium" : "text-warning font-medium"}`}
            >
...
export default function ParentPayFeesPage() {
  ...
  return (
    <div>
      <PageHeader
        title="Pay Fees"
        description="Pending fees for this child."
        breadcrumb={["Dashboard", "Fees", "Pay Fees"]}
      />
      <ChildSwitcher />
      ...
      <div className="panel">
        {childrenLoading || isLoading ? (
          ...
          <EmptyState
            ...
            description="This child has no pending fees right now."
            icon={Wallet}
          />
        ) : (
