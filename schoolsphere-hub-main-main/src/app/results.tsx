...
function StudentResultsView() {
  ...
  const rowsByTerm = useMemo(() => {
    const grouped = new Map<
      string,
      { subject: string; marks: number | null; maxMarks: number; grade: string; publishedAt: string }[]
    >();
  ...
  const availableTerms = useMemo(
    () => TERM_ORDER.filter((t) => (rowsByTerm.get(t) ?? []).length > 0),
    [rowsByTerm],
  );
  ...
  // Each subject result auto-hides 48 hours after it was published — show the soonest
  // expiry among the currently visible rows so the student knows the window is closing.
  const hoursRemaining = useMemo(() => {
    if (!activeRows.length) return null;
    const expiries = activeRows.map((r) => new Date(r.publishedAt).getTime() + 48 * 60 * 60 * 1000);
    const soonest = Math.min(...expiries);
    return Math.max(0, Math.ceil((soonest - now) / (60 * 60 * 1000)));
  }, [activeRows, now]);
  ...
  return (
    <div>
      ...
      {hoursRemaining !== null && (
        <div className="mb-4 rounded-xl border border-warning/30 bg-warning-soft px-4 py-2.5 text-sm text-warning print:hidden">
          {hoursRemaining > 0
            ? `This result is visible for a limited time and will auto-hide in ${hoursRemaining} hour${hoursRemaining === 1 ? "" : "s"}. Print or save it now if you need a copy.`
            : "This result's visibility window has ended and it will disappear shortly."}
        </div>
      )}
      ...
      {availableTerms.length > 1 && (
        <Tabs value={term} onValueChange={setTerm} className="mb-4 print:hidden">
          <TabsList>
            {TERM_ORDER.map((t) => (
              <TabsTrigger key={t} value={t} disabled={!availableTerms.includes(t)}>
      ...
      <div id="report-card" className="mx-auto max-w-3xl overflow-hidden rounded-2xl border border-border bg-surface shadow-sm print:max-w-none print:rounded-none print:border-none print:shadow-none">
        ...
        {isLoading ? (
          ...
          <EmptyState title="No results yet" description="Results for this term have not been published." />
        ) : (
      ...
      {availableTerms.length === 0 && !isLoading && (
        <SectionCard title="No published results" className="mt-4 print:hidden">
          <EmptyState title="Nothing here yet" description="Your teachers haven't published any results yet." />
        </SectionCard>
      )}
    </div>
  );
