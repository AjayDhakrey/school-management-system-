...
function StudentDashboard() {
  ...
  const upcomingExamsAll = (exams ?? []).filter((e) => e.status === "Scheduled");
        ...
        >
          {(notices ?? []).length === 0 ? (
            ...
            (notices ?? []).slice(0, 4).map((n) => (
              <div key={n.id} className="rounded-xl border border-border px-3 py-2">
...
const TEACHER_SHORTCUTS: TeacherShortcut[] = [
  ...
  { label: "My Timetable", image: timeIcon, to: "/timetable" },
  { label: "Attendance", image: attendanceIcon, to: "/attendance" },
  { label: "Homework", image: homeworkIcon, to: "/homework" },
  { label: "Exams", image: examIcon, to: "/examinations" },
    ...
    <>
        ...
        >
          {(notices ?? []).slice(0, 4).map((n, i) => (
            <article
              key={n.id}
              className={cn(
                "grid grid-cols-[auto_minmax(0,1fr)_auto] items-start gap-3 rounded-2xl border border-border p-3",
      ...
      <div className="mt-3 grid gap-3 lg:grid-cols-3">
        ...
        <SectionCard title="Upcoming Exams" subtitle="Scheduled assessments" bodyClassName="space-y-2.5">
          ...
          {canExams && (exams ?? []).filter((e) => e.status === "Scheduled").length === 0 && (
            <p className="py-4 text-center text-sm text-muted-foreground">No scheduled exams.</p>
          )}
        </SectionCard>
      </div>
