...
export default function ProfilePage() {
  ...
  if (!student) return <EmptyState title="Profile not found" description="Contact your school administrator." />;

  return (
    <div>
      <PageHeader title="My Profile" description="View and update your personal details." breadcrumb={["Dashboard", "My Profile"]} />
      ...
      <div className="mt-4 grid gap-4 lg:grid-cols-2">
        <SectionCard title="Academic & Admission Info" subtitle="Read-only — contact the school office to change">
        ...
        <SectionCard title="Contact Details" subtitle="You can update these fields">
          <form onSubmit={handleSubmit} className="grid gap-3">
            <div className="grid gap-1.5">
              <Label htmlFor="p-email"><Mail className="mr-1 inline h-3.5 w-3.5" />Email</Label>
              <Input id="p-email" type="email" value={email} onChange={(e) => setEmail(e.target.value)} />
            </div>
            <div className="grid gap-1.5">
              <Label htmlFor="p-phone"><Phone className="mr-1 inline h-3.5 w-3.5" />Phone</Label>
              <Input id="p-phone" value={phone} onChange={(e) => setPhone(e.target.value)} />
            </div>
            <div className="grid gap-1.5">
              <Label htmlFor="p-address"><MapPin className="mr-1 inline h-3.5 w-3.5" />Address</Label>
              <Input id="p-address" value={address} onChange={(e) => setAddress(e.target.value)} />
            </div>
            <div className="grid gap-1.5">
              <Label htmlFor="p-blood"><Droplet className="mr-1 inline h-3.5 w-3.5" />Blood Group</Label>
              <Input id="p-blood" value={bloodGroup} onChange={(e) => setBloodGroup(e.target.value)} placeholder="O+" />
            </div>
            <div className="grid gap-1.5">
      ...
      <SectionCard title="Parents / Guardians" subtitle="Linked guardian accounts" className="mt-4">
        {(parents ?? []).length === 0 ? (
          <EmptyState title="No guardians linked" description="Your school will link a parent/guardian account to your profile." icon={Cake} />
        ) : (
