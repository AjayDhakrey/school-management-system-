...
export default function TeacherProfilePage() {
  ...
  if (!teacher) return <EmptyState title="Profile not found" description="Contact your school administrator." />;
  ...
  const classNames = (classes ?? []).filter((c) => assignedClassIds.includes(c.id)).map((c) => `${c.name}-${c.section}`);
  const subjectNames = (subjects ?? []).filter((s) => assignedSubjectIds.includes(s.id)).map((s) => s.name);
        ...
        <div className="flex flex-col gap-4 sm:flex-row sm:items-start">
          ...
          <div className="min-w-0 flex-1">
            ...
            <p className="text-xs text-muted-foreground">
              Employee ID: {teacher.id.slice(0, 8).toUpperCase()} · {teacher.designation ?? "Teacher"}
            </p>
        ...
        <SectionCard title="Contact Details" subtitle="You can update these fields">
          <form onSubmit={handleSubmit} className="grid gap-3">
            ...
            <div className="grid gap-1.5">
              ...
              <Input id="t-photo" value={photoUrl} onChange={(e) => setPhotoUrl(e.target.value)} placeholder="https://…" />
            </div>
